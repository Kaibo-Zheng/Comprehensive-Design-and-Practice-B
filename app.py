"""Main real-time gimbal person tracking application."""
...
from core.metrics import FrameTimings, MetricsSnapshot, TrackingMetrics
...
def parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    ...
    parser.add_argument("--voice-text", default="检测到{label_zh}。")
...
def build_detector(config: DetectorConfig):
    ...
    YOLO modules are imported lazily so the OpenCV path never requires
    onnxruntime / rknnlite to be installed.
    """
...
def run(args: argparse.Namespace) -> int:
    ...
    try:
        ...
        while True:
            ...
            # Detectors may publish a per-stage breakdown via `last_timing`
            # (preprocess/inference/postprocess in ms); otherwise the whole
            # detect() call is attributed to inference_ms.
