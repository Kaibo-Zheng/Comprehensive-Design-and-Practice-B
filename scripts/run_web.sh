...
#
#   bash scripts/run_web.sh           # live; runs until Ctrl-C
#   bash scripts/run_web.sh --mock    # finite headless mock smoke (frees the port)
#
# Env overrides: CAMERA, WIDTH, HEIGHT, FPS, DETECTOR, MODEL, TARGET_CLASS,
# WEB_HOST, WEB_PORT, WEB_QUALITY, WEB_MAX_FPS, LOG, MAX_FRAMES, PYTHON, MOCK=1
set -euo pipefail
...
CMD=("$PYTHON" -m tracker.app --camera "$CAMERA" --width "$WIDTH" --height "$HEIGHT" \
  --fps "$FPS" --detector "$DETECTOR" --web-host "$WEB_HOST" \
  --web-port "$WEB_PORT" --web-quality "$WEB_QUALITY" \
  --web-max-fps "$WEB_MAX_FPS" --no-display --log "$LOG")
